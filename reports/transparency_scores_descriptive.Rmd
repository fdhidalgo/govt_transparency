...
census_merge <- select(census, STATE, PLACE, ST_FIPS) %>%
  mutate(ST_FIPS = as.numeric(ST_FIPS)) %>%
  rename(fips_state = "STATE", fips_place = "PLACE") %>%
  filter(fips_place != "00000") %>%
  distinct() 
preds_export <- left_join(preds, census_merge)

#write_csv(preds_export, "~/Downloads/transparency_scores.csv")
